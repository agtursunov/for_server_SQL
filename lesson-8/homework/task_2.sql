...
SELECT 
	...
	IntervalYear,
	CASE 
		WHEN IntervalYear < 2 THEN NULL
		ELSE CONCAT(CAST(HireYear + 1 AS NVARCHAR), ' - ', CAST(NextYear - 1 AS NVARCHAR))
	END AS Not_hired_years
FROM (SELECT 
	DISTINCT CAST(YEAR(HIRE_DATE) AS INT) AS HireYear,
